                        ...
                        <label tabindex="0" class="btn btn-ghost btn-circle">
                            <div class="indicator">
                                ...
                                @if(isset($unreadNotificationsCount) && $unreadNotificationsCount > 0)
                                    <span id="navbarNotificationBadge" class="badge badge-xs badge-error indicator-item">{{ $unreadNotificationsCount }}</span>
                                @else
                                    <span id="navbarNotificationBadge" class="badge badge-xs badge-error indicator-item hidden">0</span> {{-- Tetap ada tapi hidden --}}
                                @endif
                            </div>
                        </label>
                        <div tabindex="0" id="notificationDropdown" class="mt-3 z-[50] card card-compact dropdown-content w-64 sm:w-80 bg-base-100 shadow">
                            <div class="card-body p-0">
                                <div class="p-3 border-b border-base-300 flex justify-between items-center">
                                    ...
                                    <form method="POST" action="{{ route('user.notifications.markAllAsRead') }}" class="inline">
                                        @csrf
                                        <button type="submit" class="btn btn-xs btn-ghost text-primary p-0">Tandai semua dibaca</button>
                                    </form>
                                    @endif
                                </div>
                                <ul id="notificationList" class="menu menu-sm max-h-72 overflow-y-auto">
                                        ...
                                        @foreach($unreadNotifications as $notification)
                                            <li class="{{ $notification->read_at ? '' : 'bg-base-200 font-semibold' }}">
                                                <a href="{{ route('user.notifications.markAsRead', $notification->id) }}"
                                                   class="notification-link whitespace-normal py-3 block"> {{-- Tambah block --}}
